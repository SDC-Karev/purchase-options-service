...
const generateGames = (callback) => {
  ...
  const generateAndWrite = () => {
    ...
    do {
      ...
      const tagNumber = Math.floor(Math.random() * 20) + 1;
      const platformNumber = Math.floor(Math.random() * 3) + 1;
      ...
      data.sale_amount = faker.finance.amount() * -1;
      data.platforms = [];
      data.tags = [];

      for (let j = 1; j <= tagNumber; j += 1) {
        data.tags.push(tags[j]);
      }
      for (let k = 1; k <= platformNumber; k += 1) {
        data.platforms.push(platforms[k]);
      }

      if (i === 10000000) {
        wStream.write(`${i}|${data.game_name}|${data.game_price}|${data.game_banner}|${data.game_release_date.getFullYear()}-${data.game_release_date.getMonth() + 1}-${data.game_release_date.getDate()}|${data.dev_name}|${data.sale_amount}|${JSON.stringify(data.platforms)}|${JSON.stringify(data.tags)}\n`, (err) => {
          if (err) throw err;
          wStream.end();
        });
      } else {
        ...
        ok = wStream.write(`${i}|${data.game_name}|${data.game_price}|${data.game_banner}|${data.game_release_date.getFullYear()}-${data.game_release_date.getMonth() + 1}-${data.game_release_date.getDate()}|${data.dev_name}|${data.sale_amount}|${JSON.stringify(data.platforms)}|${JSON.stringify(data.tags)}\n`, (err) => {
          if (err) throw err;
        });
      }
      i += 1;
...
const generateBundles = (callback) => {
  ...
  const generateAndWrite = () => {
    ...
    do {
      ...
      for (let j = 1; j <= platformNumber; j += 1) {
        data.platforms.push(platforms[j]);
      }

      if (i === 30) {
        wStream.write(`${i}|${data.bundle_name}|${data.bundle_price}|${data.sale_amount}|${JSON.stringify(data.platforms)}\n`, (err) => {
          if (err) throw err;
          wStream.end();
        });
      } else {
        ...
        ok = wStream.write(`${i}|${data.bundle_name}|${data.bundle_price}|${data.sale_amount}|${JSON.stringify(data.platforms)}\n`, (err) => {
          if (err) throw err;
        });
      }
      i += 1;
...
const generateGamesBundlesJoin = (callback) => {
  ...
  const generateAndWrite = () => {
    ...
    do {
      ...
      data.bundle_id = Math.floor(Math.random() * 3000000) + 1;
